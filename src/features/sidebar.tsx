...
export const MainLayout = () => {
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen w-full bg-[#030712] overflow-hidden text-white font-sans">
      {/* 1. TOPBAR MOBILE (Somente visível em telas pequenas) */}
      <header className="flex md:hidden items-center justify-between px-6 py-4 border-b border-gray-800 bg-[#111827]/50 backdrop-blur-md sticky top-0 z-50">
        <h1 className="text-lg font-black italic uppercase tracking-tighter text-blue-500">
          {menuItems.filter((p) => p.url === location.pathname)[0].label}
        </h1>
        <Avatar
          src={session?.user.user_metadata.avatar_url}
          sx={{
      ...
      {/* 2. SIDEBAR DESKTOP (Escondida em mobile: hidden, visível em md:flex) */}
      ...
      {/* 3. CONTEÚDO PRINCIPAL (Ajuste de padding para não bater na barra inferior no mobile) */}
