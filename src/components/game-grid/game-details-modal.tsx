  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center animate-in fade-in duration-300">
      ...
      <div className="relative w-full h-full md:h-auto md:max-w-4xl md:max-h-[85vh] bg-gray-950 md:border md:border-white/10 md:rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row animate-in md:zoom-in-95 duration-300">
        ...
        <div className="w-full md:w-5/12 relative shrink-0 h-[40vh] md:h-auto">
          <img
            src={game.coverUrl}
            alt={game.name}
        ...
        <div className="w-full md:w-7/12 p-6 md:p-10 flex flex-col gap-6 md:gap-8 overflow-y-auto custom-scrollbar">
          
          <div className="space-y-4">
            <h2 className="text-3xl md:text-5xl font-black text-white leading-[1.1] italic uppercase tracking-tighter break-words">
              {game.name}
            </h2>
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {game.platforms?.map((plat) => (
                  <span
                    key={plat.id}
                    className="px-3 py-1.5 bg-white/5 border border-white/10 rounded-md text-[9px] font-bold text-gray-400 uppercase tracking-tight"
                  >
                    {plat.name}
                  </span>
                ))}
          ...
          <div className="mt-auto pt-6 border-t border-white/5 flex flex-col gap-2">
             <p className="text-gray-600 text-[9px] leading-relaxed uppercase font-medium tracking-widest">
              Dados via IGDB API • {game.id}
            </p>
          </div>
        </div>
      </div>
